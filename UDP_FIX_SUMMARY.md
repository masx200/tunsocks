...
## Problem
The original tunsocks implementation had a critical flaw in SOCKS5 UDP ASSOCIATE support:
...
- Clients could only send UDP packets to one destination, not multiple destinations
...
### Verify
The binary should now properly handle SOCKS5 UDP ASSOCIATE requests from multiple clients to multiple destinations.
...
## Notes
- This fix is essential for applications that need to send UDP packets to multiple destinations
